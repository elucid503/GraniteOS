...
const ELF_MAGIC: u32 = 0x464C457F; // 0x7F or 'E' 'L' 'F'
...
/// Validates the ELF, maps pages into l0_pa, copy segments, and then finally allocates user stack.
pub fn load(elf_bytes: []const u8, l0_pa: usize) LoadError!LoadResult {
    ...
    // Use align(1) to avoid a runtime alignment fault when the embedded ELF bytes are not naturally aligned to ElfHeader's alignment (8 bytes for u64 fields).
...
fn load_segment(elf_bytes: []const u8, ph: *align(1) const ProgramHeader, l0_pa: usize) LoadError!void {
    ...
    // Time to zero the full virtual extent, then copy file image in.
    ...
    // Cleans D-cache to Point of Unification so the I-cache sees the written code. Necessary to avoid stale instruction fetches after loading new code.
